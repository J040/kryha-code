...
function findDangerousCoordinates() {
  ...
  for (let coordinate of coordinates) {
    ...
    if (coordinate['x1'] == coordinate['x2']) { // VERTICAL LINE
    ...
    } else if (coordinate['y1'] == coordinate['y2']) { // HORIZONTAL LINE
      ...
      if (coordinate['x1'] < coordinate['x2']) {
        if (coordinate['y1'] < coordinate['y2']) {
          while (y <= coordinate['y2'] && x <= coordinate['x2']) {
            matrix[y][x] +=1;
            y++; x++;
          }
        } else {
  ...
  for (let y = 0; y < matrix.length; y++) {
    for (let x = 0 ; x < matrix[y].length; x++) {
      if (matrix[y][x] > 1) {
        console.log('danger at: [', y, '][', x, '] ', matrix[y][x])
